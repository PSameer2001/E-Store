...
const AddToCart = (props) => {
  ...
  const getCartData = async (email) => {
    const res = await axios.post(`${process.env.SERVER_URL}/getCartData`, {
      email: email,
    });
    const data = res.data;
    setCart(data);
  ...
  const getUserCouponData = async (email) => {
    const res = await axios.post(`${process.env.SERVER_URL}/getUserCouponData`, {
      email: email,
    });
    const data = res.data;
    setCoupon(data);
  ...
  //   const res = await axios.get(`${process.env.SERVER_URL}/getallCouponData`);
  //   const data = res.data;
  //   setAllCoupon(data);
  // };
  ...
  const removeFromCart = async (id) => {
    try {
      removeCoupon();
      const res = await axios.post(`${process.env.SERVER_URL}/removeFromCart`, { id });
  ...
  const handleChangeQuantity = async (id, quantity) => {
    ...
    if (couponAmount !== 0) {
      await axios.post(`${process.env.SERVER_URL}/removeCoupon`, {
        email: user.email,
      });
  ...
  useEffect(() => {
    if (cartId !== "" && cartQuantity !== 0) {
      const timer = setTimeout(async () => {
        const res = await axios.post(`${process.env.SERVER_URL}/updateQuantityofProduct`, {
          id: cartId,
  ...
  const removeCoupon = async () => {
    try {
      const res = await axios.post(`${process.env.SERVER_URL}/removeCoupon`, {
        email: user.email,
      });
  ...
  const applyCoupon = async () => {
    try {
      ...
      const res = await axios.post(`${process.env.SERVER_URL}/applyCoupon`, {
  ...
  const initPay = (data) => {
    const options = {
      ...
      handler: async (response) => {
        try {
          const res = await axios.post(`${process.env.SERVER_URL}/verifyPayment`, {
            response,
            cart,
            finalAmount,
            phone: user.phone,
  ...
  const checkoutProduct = async () => {
    try {
      const { data } = await axios.post(`${process.env.SERVER_URL}/checkoutProduct`, {
        finalAmount,
      });
      // console.log(data);
      initPay(data.data);
