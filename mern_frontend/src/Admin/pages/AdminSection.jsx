...
const AdminSection = () => {
  ...
  const getallSectionData = async () => {
    const res = await axios.get(`${process.env.SERVER_URL}/getallSectionData`);
    const data = res.data;
    setRows(data);
  };
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault();
      setIsLoading(true);
      const res = await axios.post(`${process.env.SERVER_URL}/addSection`, sectiondata);
  ...
  const handledeleteSection = async (id) => {
    try {
      const res = await axios.post(`${process.env.SERVER_URL}/deleteSection`, { id });
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      setIsLoading(true);
      const res = await axios.post(`${process.env.SERVER_URL}/editSection`, editsectiondata);
      let message = res.data.message;
